...
1. **`analysis_0.csv`**: Focuses on the evolution of peer influence and sensitivity.
2. **`analysis_1.csv`**: Investigates the evolution of learning portfolios.
3. **`analysis_2.csv`**: Examines parochialism and the effects of stratified populations.
...
- **`u`**: Environmental success probability.
- **`aleph`**: Wealth buffer values.
- **`mu_std`**: Mutation rate for sensitivity to ruin.
- **`mu_soc_h`**: Mutation rate for horizontal social learning.
- **`mu_soc_v`**: Mutation rate for vertical social learning.
...
2. **Analysis 1: Evolution of Learning Portfolios**
   - **`m`**: Number of elders sampled for learning.
   - **`envshift`**: Frequency of environmental change.
   - **`periodic`**: Boolean for periodic environmental changes.
   - **`peg_aleph`**: Boolean indicating if wealth buffers are pegged to environmental shifts.
   - **`u_shift`**: Shift values for environmental success probabilities.
