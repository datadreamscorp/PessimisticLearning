...
# ╔═╡ d67ffc07-3bda-4d93-b33a-758f6e54c3ed
md"""

Model steps:

1. **First cohort is generated**, becoming current cohort. $N$ individuals with $s_i \in (0, 1)$, $l_i \in (0,1)$ and $\delta_i \sim \text{Exponential}(0.001)$. They are all given the same $\nu$ and $\tau$. All within-cohort learners, as there are no cohorts generations to learn from, so every $L_i = 0$ and $\zeta_i = 0$.


2. **Current cohort learns.** 

For every agent in the current cohort, if $L_i = 0$:

- Individual learning: the agent throws a coin $\tau$ times, takes the average and passes it through the behavioral filter, producing an estimate $\hat{s}_i$.

- Horizontal learning: the agent chooses $n_H$ other agents from the cohort and averages their estimate with theirs (weighted by their reliance on horizontal learning $l_i$), obtaining $\bar{s}_i = l_i \hat{s}_i + (1 - l_i) \frac{1}{n} \sum_{j = 1}^n \hat{s}_j$.

Otherwise, if $L_i = 1$:

- The agent chooses $n_V$ other agents from the previous cohort to build a learning target pool $M_i$. If $D = 1$, the learning pool is demographically-filtered i.e. only agents with no losses are eligible to be chosen as target candidates.

- Vertical learning occurs. Each agent chooses a previous cohort agent to imitate with error. If $S_i = 0$, the choice is done at random (unbiased learning). Otherwise, $S_i = 1$ and the agent chooses a target $m \in M_i$ with probability

$\text{P}_i(m) = \frac{w(m)_i}{w(m)_i^\text{max}}$

where $w_i(m) = Κ_i(s_m) \cdot v_m$ is a frequency-weighted payoff, and

$K_i(s) = \text{exp} \bigg( \frac{(s - M[S])^2}{\zeta_i} \bigg)$

is the frequency-biased weight that agent $i$ gives to an $s \in M_i$, $\zeta_i > 0$ is $i$'s frequency bias, and $v_m$ is the payoff of target candidate $m$.

3. **Current cohort reproduces.**

$N$ new agents are generated, and they inherit a previous cohort agent's $\delta$, 
...
begin
	...
	plot!(
		xaxis,
		optimal_fraction.(xaxis) .* expected_stake.(xaxis),
		label="optimal stake",
		lw=2,
		color="black"
	)
...
begin
	Random.seed!(12345)
	est01 = estimate_plots(1.0, tries[3], n, title=true, label=true)
	est02 = estimate_plots(1.5, tries[3], n, title=false)
	est03 = estimate_plots(2.0, tries[3], n, title=false)
	plot(
		[est01..., 
		est02..., 
		est03...]..., 
...
begin
	...
	for t in tries
		mstake = meanstake.(20000, t, xaxis)
		smean = stakemean.(20000, t, xaxis)
...
plot_meanconf([1.5, 3.0, 4.5], [0, 1, 5])
...
function simulate_gambles_alterioni(λ, stake, n;
	sens=0.01,
	u=1.0,
	init_capital=1.0,
	seasons=2000,
...
begin
	Random.seed!(123456)
	seedlers = rand(Xoshiro(573563), 1:100000, 100)
	log_payoffs, stakes = simulate_gambles_alterioni(2.0, 0.5, 100)
	pays_adaptive = exp.(log_payoffs ./ 2000)
	pays = [exp( simulate_gambles(1.0, 2.0, 0.5) ./ 2000 ) for seed in seedlers]
...
function compare_learning(
	...
	for i in 1:total_samples
		...
		lpays, stakers = simulate_gambles_alterioni(
			λ, meaner, n_samples, 
...
# ╔═╡ 91f2fe92-0fdb-48ca-9d74-99eb246c6835
begin
	capi = 1
	lamb = 2.0
end

# ╔═╡ 47f0f6ad-28fe-4edc-8293-b6a445ca06dc
pfs, medbias, mdev, paybias, pdev = compare_learning(lamb, capi, 1000, 5, 50, 0.07)

# ╔═╡ 1e6eae1b-8fe7-4a11-99e3-c19fb4daf713
begin
	histogram(mdev, c="pink")
	histogram!(pdev, alpha=0.5, c="dark blue")
end

# ╔═╡ f528f7eb-728d-49db-8c19-31ce64aa1da2
scatter(
	[1-0.05, 1+0.05],
	[mean(mdev), mean(pdev)],
	yerr=[std(mdev), std(pdev)],
	c=["pink", "dark blue"],
	ylim=(-0.2,0.25),
	xlim=(0, 5)
)

# ╔═╡ 3b6f38ee-0ae9-4e47-bd40-08ae5b52378d
survpays_med, fullpays_med, probsurv_med = sim_payoffs(2.0, medbias, 1)

# ╔═╡ 176c6fea-4186-4276-af12-f3d10e2721d8
survpays_pay, fullpays_pay, probsurv_pay = sim_payoffs(2.0, paybias, 1)

# ╔═╡ 478dc155-3ff6-45f9-9b34-6440243f3ca3
begin
	histogram(fullpays_med, color="pink")
	histogram!(fullpays_pay, color="dark blue", alpha=0.5)
end

# ╔═╡ 1f03846f-834f-4a22-8712-ecce9a0e3a0b
median(fullpays_med.^100)

# ╔═╡ 34ddbf62-c315-4327-8771-27acbeef9de4
median(fullpays_pay.^100)

# ╔═╡ 9cb067fa-4ab2-41cf-9688-eff29b8271ef
function plot_sensitivity(λ, cap, N, tries, T; succ_bias=false, n_samples=1000)
...
# ╔═╡ 420fac1b-2910-4810-b4fb-136658ffaf5d
plot_sensitivity([1.5, 2.0, 3.5, 5.0], 1, 50, 5, 1000; succ_bias=false, n_samples=1000)

# ╔═╡ ea14eddb-8913-4eb7-b36b-bcac9931190d
#=╠═╡
begin
	#adaptpes, adaptstakes = simulate_gambles_alt(lp, opt_lp+0.1, 1000, sens=0.01)
	wealthgambles = simulate_gambles_adaptive.(lp, opt_lp, 1000, 0.0:0.01:1.0, -2, true_random=false)
	plot(
		0.0:0.01:1.0,
		mean.([exp.(p[1] ./ 2000) for p in wealthgambles]),
		label="wealth-dependent stake",
		xlab="wealth sensitivity",
		ylab="mean payoff",
		lw=2,
		color="black",
		legend=:right
	...
	hline!(
		[findmax(adapt_payoffs)[1]],
		lw = 2,
		label="optimal adaptive pessimism",
		color="black",
		ls=:dash
	)
end
